...
import string
from reg_mail_tm import regMail, get_messages_code, get_domain
from reg_tempmail import create_temp_mail, get_temp_mail_messages, get_temp_mail_domains
...
class MainHandler(Ui_MainWindow, QtWidgets.QMainWindow):
  def __init__(self):
  ...
  def changeSite(self):
    if self.cbSite.currentText() == "mail.tm":
       ...
       self.cbDomain.clear()
       self.cbDomain.addItem(domain)
    elif self.cbSite.currentText() == "temp-mail.io":
...
class mainThread(QThread):
  ...
  def __init__(self, mainHandler: MainHandler):
  ...
  def run(self):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        for i in range(num_threads):
            ...
            future = executor.submit(self.regAccountCC, emailSource)
  ...
  def random_password(self, length=7):
  ...
  def updateAccountReg(self, account):
    with open('account_reg.txt', 'a') as f:
      f.write(account + '\n')

  def changIpByExpressVPN(self):
      try:
  ...
  def register_verify_login(self, email_encode, password_encode, code_encode, userAgent=None):
      if not userAgent:
        userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Safari/537.36"
    ...
    if emailSource == 'mail.tm':
        while True:
          try:
            ...
            email = regMail(userAgent, self.mainHandler.cbDomain.currentText())
            password = self.random_password()
            # print(email + '|' + password)
            email_encode = self.encode_email(email)
            password_encode = self.encode_password(password)
            result = self.registerCC(email_encode, password_encode, userAgent)
            if result != 'success':
                if changIP == False:
                    with lock:
                      changIP = True
            ...
            for i in range(10):
              ...
              if code and code != 'watting':
                ...
                userID = self.register_verify_login(email_encode, password_encode, code_encode, userAgent)
                if userID:
        ...
        while True:
          try:
            userAgent = self.getUserAgent()
            emailName = self.getRandomEmail()
            payload = {
                "domain": self.mainHandler.cbDomain.currentText(),
                "name": emailName
            }
            email = create_temp_mail(userAgent, payload)
            password = self.random_password()
            # print(email + '|' + password)
            email_encode = self.encode_email(email)
            password_encode = self.encode_password(password)
